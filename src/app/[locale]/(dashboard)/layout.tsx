...
import { LayoutDashboard, Building2, Users, FileText, Settings, LogOut, Receipt, Gift, PieChart } from "lucide-react";
            ...
            <nav className="mt-8 flex-1 px-2 space-y-1">
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={`${isActive ? "text-primary dark:text-green-400" : "text-gray-400 dark:text-gray-500"} mr-3 flex-shrink-0 h-5 w-5`} />
                    {item.name}
                  </Link>
                );
              })}
      ...
      <div className="md:pl-64 flex flex-col flex-1">
        ...
        <div className="sticky top-0 z-10 md:hidden flex-shrink-0 flex h-16 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 items-center justify-between px-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={handleLanguageSwitch} className="text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-md">
              {locale === "en" ? "বাংলা" : "EN"}
            </button>
            <button onClick={() => signOut({ callbackUrl: `/${locale}/login` })} className="p-2 text-gray-400 hover:text-red-500">
        ...
        <nav className="md:hidden fixed bottom-0 left-0 right-0 z-10 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800 flex justify-around py-2">
          {navigation.slice(0, 5).map((item) => {
            const isActive = pathname === item.href || (item.href !== `/${locale}` && pathname.startsWith(item.href));
            return (
              <Link key={item.href} href={item.href} className={`flex flex-col items-center px-2 py-1 ${isActive ? "text-primary" : "text-gray-400"}`}>
